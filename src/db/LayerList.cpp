...
void LayerList::initViaWire(const int layerIdx,
    ...
    for (unsigned i = 0; i != layer.numCrossPoints(); ++i) {
        ...
        for (unsigned j = 0; j != xSize * 2 + 1; ++j) {
            DBU xDist = 0;
            if (j < xSize) {
                xDist = max(0LL, int(xSize - j) * layer.pitch - halfWidth + viaMetal[layer.direction].low);
            } else if (j > xSize) {
                xDist = max(0LL, int(j - xSize) * layer.pitch - halfWidth - viaMetal[layer.direction].high);
            }
            for (unsigned k = max(0, (int)ySize - (int)i); k < min(ySize * 2 + 1, layer.numCrossPoints() + ySize - i);
                ...
                DBU yDist = 0;
                if (k < ySize) {
                    yDist = max(0LL, cp.location - tmpCP.location - halfWidth + viaMetal[1 - layer.direction].low);
                } else if (k > ySize) {
                    yDist = max(0LL, tmpCP.location - cp.location - halfWidth - viaMetal[1 - layer.direction].high);
                }
